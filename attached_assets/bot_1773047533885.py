...
MAIN_CHANNEL_ID = "-1003181034907"      # Основной канал
MOD_CHANNEL_ID = "-1003278302331"       # Канал для скрытой модерации
ADMIN_ID = 6437612855                    # Твой Telegram ID

active_chats = set()
pending_messages = {}  # chat_id -> сообщение пользователя (Update.message)
mod_messages = {}      # chat_id -> dict с текстом и типом медиа
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Привет! Напиши текст, фото, видео или гиф и выбери как его отправить анонимно."
    )
...
async def moderation_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.from_user.id != ADMIN_ID:
        await query.answer("❌ Только админ может модерацию", show_alert=True)
        return
    ...
    text = msg_data.get("text", "(текст не найден)")
...
if __name__ == "__main__":
    ...
    print("Бот с модерацией и поддержкой медиа запущен...")
    app.run_polling()